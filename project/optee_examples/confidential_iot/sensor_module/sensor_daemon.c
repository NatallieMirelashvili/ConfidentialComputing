 ...
 *
 * Holds its own copy of the sensor<->device pre-shared secret, loaded from
 * a local file written by scripts/pair-sensor.sh - never compiled in, so
 * one built image can be paired with any device without a rebuild.
 */
